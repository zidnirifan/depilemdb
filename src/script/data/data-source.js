...
const removeJumbotron = () => {
  document.querySelector(".jumbotron").classList.add("d-none");
  document.querySelector(".navbar").classList.add("bg-custom");
};

const imageError = () => {
  const images = document.querySelectorAll(".image");
  images.forEach((img) => {
    if (img.firstElementChild.src == "http://image.tmdb.org/t/p/w500null") {
      img.innerHTML = `<svg xmlns="http://www.w3.org/2000/svg" width="160" height="240" viewBox="0 0 160 240">
...
const resultsMovies = (movie) => {
  return `<movie-item
                ...
                class="card mt-2 mb-2 my-4 col-lg-2 col-md-3 col-sm-4 col-5">
            </movie-item>`;
};

const movieDetails = (selector) => {
  const button = document.querySelectorAll(selector);
  button.forEach((btn) => {
    btn.addEventListener("click", function () {
      removeJumbotron();
      let idMovie = this.dataset.idmovie;
      fetch(`${baseUrl}/movie/${idMovie}?${apiKey}`)
        .then((response) => response.json())
        .then((movie) => {
          const movieDetail = movieInfo(movie);
          document.querySelector("#content").innerHTML = movieDetail;
          listMovies("/recommendations", ".recommendations", `/${idMovie}`);
          listMovies("/similar", ".similar", `/${idMovie}`);
          trailer(idMovie);
        })
        .catch((error) => {
          removeJumbotron();
...
const movieInfo = (movie) => {
  ...
  return `<movie-details 
                ...
                country="${movie.production_companies[0].origin_country}" 
                genres="${genres}"  
                overview="${movie.overview}">
            </movie-details>
            <h4 class="text-custom mt-5 ml-3">Recommendations</h4>
            <div class="line ml-3"></div>
            <div class="column-flex recommendations"></div>
            <h4 class="text-custom mt-5 ml-3">Similar Movies</h4>
            <div class="line ml-3"></div>
            <div class="column-flex similar"></div>`;
};

const trailer = (idMovie) => {
  document.querySelector("#btn-trailer").addEventListener("click", () => {
    fetch(`${baseUrl}/movie/${idMovie}/videos?${apiKey}`)
      .then((response) => response.json())
      .then((video) => {
        if (video.results.length == 0) {
          document.querySelector(".trailer").innerHTML = `${requestError}
                                    <h3 class="text-muted text-center mb-5">Trailer not found<h3>`;
        } else {
          const idVideo = video.results[0].key;
